...
function findClassMethod(node, className, methodName) {
    ...
    return (ts.forEachChild(node, (childNode) => findClassMethod(childNode, className, methodName)) || "");
}
/**
...
function findClassProperty(node, className, propertyName) {
    if (ts.isClassDeclaration(node) &&
        node.name &&
        node.name.getText() === className) {
        for (const member of node.members) {
    ...
    return (ts.forEachChild(node, (childNode) => findClassProperty(childNode, className, propertyName)) || "");
}
